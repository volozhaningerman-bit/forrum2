...
try {
  ...
  assert.equal(await page.locator('.office-room-links>button').count(), 5);
  assert.equal(await page.getByRole('button', { name: /Задачи/ }).count() >= 1, true);
  ...
  assert(initialGameRect && initialGameRect.y >= 0 && initialGameRect.y + initialGameRect.height <= 1001);
  assert.equal(await page.evaluate(() => window.scrollY), 0);
  assert.equal(await page.locator('.office-right').evaluate((node) => getComputedStyle(node).overflowY), 'hidden');
  assert.equal(await page.locator('.office-profile').evaluate((node) => getComputedStyle(node).overflowY), 'hidden');
  ...
  const itemList = page.locator('.office-v6-item-list');
  const scrollable = await itemList.evaluate((node) => node.scrollWidth > node.clientWidth);
  assert.equal(scrollable, true);
  await itemList.dispatchEvent('wheel', { deltaY: 420, deltaX: 0 });
  await page.waitForTimeout(80);
  assert((await itemList.evaluate((node) => node.scrollLeft)) > 0);
  ...
  // Energy actions live on one dedicated tasks page.
  await page.getByRole('button', { name: /Задачи/ }).first().click();
  await page.getByRole('heading', { name: 'Рабочий день' }).waitFor();
  assert.equal(await page.locator('.office-v65-task-card').count(), 4);
  assert.match(await page.locator('.office-v65-task-tip').textContent(), /Предметы теперь важны/i);
  ...
  // Complete the first day from the single Tasks section, then fight the boss on its own page.
  await page.getByRole('button', { name: /Задачи/ }).first().click();
  for (const choiceName of [/Разобрать по приоритетам/, /Попросить помощи/, /Согласиться переделать/]) {
    await page.locator('.office-v65-task-card').filter({ hasText: /Первый рабочий день|Рабочие задачи/ }).click();
  ...
  await page.getByRole('heading', { name: 'Офисные испытания' }).waitFor();
  assert.equal(await page.locator('.office-v65-boss-actions>button').count(), 3);
  assert.equal(await page.getByRole('dialog', { name: /Босс:/ }).count(), 0);
  await page.getByRole('button', { name: /Аргументировать/ }).click();
  assert.match(await page.locator('.office-v65-boss-health').textContent(), /Терпение/);

  await page.screenshot({ path: output + '/bosses-v65-1600.png', fullPage: false });
  ...
  for (const width of [1600, 1200, 1024]) {
    ...
    const dimensions = await page.evaluate(() => {
      ...
      return {
        ...
        pageOverflow: getComputedStyle(document.querySelector('.office-page')).overflowY,
        rightOverflow: getComputedStyle(document.querySelector('.office-right')).overflowY,
        profileOverflow: getComputedStyle(document.querySelector('.office-profile')).overflowY,
      };
    });
    assert(
    ...
    assert.equal(dimensions.pageOverflow, 'hidden');
    assert.equal(dimensions.rightOverflow, 'hidden');
    assert.equal(dimensions.profileOverflow, 'hidden');
    assert(
  ...
  console.log('Office v6.5: one-screen home, wheel equipment, tasks, bosses and RPG pages passed');
} finally {
  await browser?.close();
  web.kill('SIGTERM');
  upstream.close();

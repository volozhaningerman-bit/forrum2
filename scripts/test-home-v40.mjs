...
const upstream = createServer(async (req,res) => {
 ...
 else if(url.pathname==='/v1/events')data=[{id:'event-1',title:'Встреча участников',startsAt:new Date().toISOString()}];
...
try {
 ...
 await more.click();await first.getByRole('button',{name:'В избранное',exact:true}).click();await first.getByText('Добавлено в избранное',{exact:true}).waitFor();assert(saved);
 await more.click();await first.getByRole('button',{name:'Убрать из избранного',exact:true}).click();await first.getByText('Убрано из избранного',{exact:true}).waitFor();assert(!saved);

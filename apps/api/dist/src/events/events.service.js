...
import { CommunityEventStatus, CommunityRoleType, EventAttendanceStatus, GlobalRole } from '../generated/prisma/client.js';
...
let EventsService = class EventsService {
    ...
    async communityWithPermission(user, slug) {
        ...
        if (!role)
            throw new ForbiddenException('События создаёт команда сообщества');
        return community;
    }
    map(event) {
        const counts = { going: 0, interested: 0 };
        for (const attendance of event.attendees ?? []) {
            if (attendance.status === EventAttendanceStatus.GOING)
                counts.going += 1;
            if (attendance.status === EventAttendanceStatus.INTERESTED)
                counts.interested += 1;
        }
        return {
    ...
    async get(id, userId) {
        ...
        if (!event || event.status === CommunityEventStatus.DRAFT && event.createdById !== userId)
    ...
    async create(user, dto) {
        ...
        if (startsAt.getTime() < Date.now() + 30 * 60_000)
            throw new BadRequestException('Событие должно начинаться не раньше чем через 30 минут');
        if (startsAt.getTime() > Date.now() + 366 * 86_400_000)
            throw new BadRequestException('Событие нельзя планировать дальше чем на год');
        if (endsAt && endsAt <= startsAt)
            throw new BadRequestException('Окончание должно быть позже начала');
        if (dto.format !== 'ONLINE' && !dto.location?.trim())
            throw new BadRequestException('Для офлайн- или гибридного события укажите место');
        ...
        if (status === CommunityEventStatus.PUBLISHED)
            void this.notifications.deliverToCommunitySubscribers({ communityId: community.id, importance: 'IMPORTANT', title: `Событие в ${community.name}`, body: event.title, href: `/events/${event.id}`, actorId: user.id }).catch(() => undefined);
        return { id: event.id };
    }
    async attendance(userId, eventId, status) {
        ...
        if (!event)
            throw new BadRequestException('Запись на событие закрыта');
        ...
        if (!attendanceRule.allowed)
            throw new BadRequestException(attendanceRule.reason === 'full' ? 'Все места уже заняты' : 'Запись на событие закрыта');
    ...
    async cancel(user, eventId, reason) {
        ...
        if (event.status === CommunityEventStatus.CANCELLED || event.status === CommunityEventStatus.COMPLETED)

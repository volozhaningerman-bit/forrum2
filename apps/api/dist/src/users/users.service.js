...
import { NotificationType, ReactionType, WallPrivacy } from '../generated/prisma/client.js';
...
import { determineLocalTrust } from './trust.js';
const helpfulReactionTypes = [ReactionType.USEFUL, ReactionType.THANKS];
const trustRank = { NEW: 0, BASIC: 1, STABLE: 2, HIGH: 3, EXCEPTIONAL: 4 };
let UsersService = class UsersService {
    ...
    async activity(userId) {
        ...
        return rows.flatMap((row) => {
            const descriptor = describeActivity(row.action, row.metadata);
            return descriptor ? [{ id: row.id, action: row.action, createdAt: row.createdAt, ...descriptor }] : [];
        });
    }
    async syncAutomaticAchievements(userId) {
        ...
        await Promise.all(achievementDefinitions.map((definition) => this.prisma.achievementDefinition.upsert({
            ...
            update: {
                ...
                category: definition.category, automatic: definition.automatic, sortOrder: definition.sortOrder, active: true,
            },
            create: {
                code: definition.code, title: definition.title, description: definition.description, icon: definition.icon,
                category: definition.category, automatic: definition.automatic, sortOrder: definition.sortOrder,
            },
        })));
    ...
    async localTrust(userId, emailVerified) {
        ...
        const ensure = (communityId, slug, name) => {
        ...
        for (const row of publications)
            ensure(row.communityId, row.community.slug, row.community.name).publicationCount += 1;
        for (const row of comments)
            ensure(row.publication.communityId, row.publication.community.slug, row.publication.community.name).commentCount += 1;
        for (const row of publicationHelpful)
            ensure(row.publication.communityId, row.publication.community.slug, row.publication.community.name).helpfulReactionCount += 1;
        for (const row of commentHelpful)
            ensure(row.comment.publication.communityId, row.comment.publication.community.slug, row.comment.publication.community.name).helpfulReactionCount += 1;
        for (const row of bookmarks)
            ensure(row.publication.communityId, row.publication.community.slug, row.publication.community.name).bookmarkCount += 1;
        for (const row of roles)
            ensure(row.communityId, row.community.slug, row.community.name).activeRole = true;
    ...
    async getProfile(usernameInput, viewerId) {
        ...
        for (const group of reviewCounts)
            reviewSummary[group.verdict.toLowerCase()] = group._count._all;
    ...
    async follow(usernameInput, followerId) {
    ...
    async unfollow(usernameInput, followerId) {
        const user = await this.prisma.user.findUnique({ where: { username: usernameInput.toLowerCase() } });
        if (!user)
            throw new NotFoundException('Пользователь не найден');
    ...
    async createWallPost(usernameInput, authorId, bodyInput) {
        const profile = await this.prisma.user.findUnique({ where: { username: usernameInput.toLowerCase() } });
        if (!profile)
            throw new NotFoundException('Пользователь не найден');
        const body = bodyInput.trim();
        if (profile.id !== authorId) {
            if (profile.wallPrivacy === WallPrivacy.ONLY_ME)
                throw new BadRequestException('Пользователь закрыл стену');
            if (profile.wallPrivacy === WallPrivacy.FOLLOWERS) {
                ...
                if (!follows)
                    throw new BadRequestException('Писать на стене могут только подписчики');
            }
        }
        const post = await this.prisma.$transaction(async (tx) => {
            const created = await tx.wallPost.create({ data: { profileUserId: profile.id, authorId, body } });
            if (profile.id !== authorId)
                await this.notifications.deliver({ userId: profile.id, actorId: authorId, type: NotificationType.WALL_POST, title: 'Новая запись на стене', body: excerpt(body, 120), href: `/u/${profile.username}` }, tx);
            return created;
        });
        return { id: post.id };
    }
    async deleteWallPost(usernameInput, postId, actorId) {
        ...
        if (!post)
            throw new NotFoundException('Запись на стене не найдена');
        if (post.authorId !== actorId && profile.id !== actorId)
            throw new ForbiddenException('Нет права удалить эту запись');

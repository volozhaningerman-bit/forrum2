...
let AdminService = class AdminService {
    ...
    config;
    moderation;
    constructor(prisma, wallet, config, moderation) {
        this.prisma = prisma;
        this.wallet = wallet;
        this.config = config;
    ...
    async resolveReport(id, status, actorId, note) {
    ...
    async hidePublication(slug, reason, actorId) {
    ...
    async hideComment(id, reason, actorId) {
    ...
    async grantCommunityRole(actorId, dto) {
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            if (existing && !existing.endedAt)
                throw new BadRequestException('У пользователя уже есть эта действующая роль');
            const role = existing
    ...
    async endCommunityRole(actorId, id, note) {
        const role = await this.prisma.communityRole.findUnique({ where: { id } });
        if (!role)
            throw new NotFoundException('Роль не найдена');
        if (role.endedAt)
            throw new BadRequestException('Роль уже завершена');
        await this.prisma.$transaction([
    ...
    async updatePromotionSettings(actorId, dto) {
        const value = { ...dto };
        await this.prisma.$transaction([
    ...
    refundPromotion(actorId, id, reason) {
        return this.wallet.adminRefund(actorId, id, reason);
    }
    grantBalance(actorId, username, amount, description) {
        return this.wallet.grant(actorId, username, amount, description);
    }
};
AdminService = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [PrismaService, WalletService, ConfigService, ModerationService])
], AdminService);
export { AdminService };

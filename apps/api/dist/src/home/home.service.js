...
function readOnlineRecord(value) {
...
let HomeService = class HomeService {
    ...
    async overview(userId) {
        ...
        const [verifiedUsers, activeUsers24h, publications, comments24h, communities, topics, messages, onlineSessions, recordSetting, weeklyTopics, weeklyComments, publicationReactions, commentReactions, polls, proposal,] = await Promise.all([
        ...
        const discussedSince = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
        ...
        const savedRecord = readOnlineRecord(recordSetting?.value);
        ...
        return {
            ...
            activePolls: polls.map((poll) => ({
                ...
                options: poll.options.map((option) => ({
                    ...
                    bindingVotes: option.votes.filter((vote) => vote.voteClass === VoteClass.BINDING).length,
                    advisoryVotes: option.votes.filter((vote) => vote.voteClass === VoteClass.ADVISORY).length,
                })),
            })),
            poll: polls[0]
                ? {
                    ...
                    totalVotes: polls[0].options.reduce((sum, option) => sum + option.votes.length, 0),

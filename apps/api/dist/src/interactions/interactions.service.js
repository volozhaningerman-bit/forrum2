...
let InteractionsService = class InteractionsService {
    ...
    async create(actorId, dto) {
        ...
        if (target.id === actorId)
            throw new BadRequestException('Нельзя создать взаимодействие с самим собой');
        ...
        if (dto.publicationSlug && (!publication || publication.status !== 'PUBLISHED'))
            throw new BadRequestException('Связанная публикация недоступна');
        ...
        if (dto.portfolioItemId && (!portfolioItem || !canLinkPortfolioInteraction({ itemStatus: portfolioItem.status, itemOwnerId: portfolioItem.ownerId, targetUserId: target.id })))
            throw new BadRequestException('Связанный проект или услуга недоступны');
        ...
        if (existing)
            throw new BadRequestException('Похожее взаимодействие уже ожидает подтверждения или завершения');
    ...
    async confirm(id, actorId) {
        ...
        if (!row)
            throw new NotFoundException('Взаимодействие не найдено');
        if (row.status !== InteractionStatus.PENDING)
            throw new BadRequestException('Это взаимодействие уже обработано');
        if (row.counterpartId !== actorId)
            throw new ForbiddenException('Подтвердить может только приглашённый пользователь');
    ...
    async complete(id, actorId) {
        ...
        if (!row)
            throw new NotFoundException('Взаимодействие не найдено');
        if (row.status !== InteractionStatus.CONFIRMED)
            throw new BadRequestException('Сначала оба участника должны подтвердить взаимодействие');
        if (row.createdById !== actorId && row.counterpartId !== actorId)
            throw new ForbiddenException('Нет доступа к этому взаимодействию');
    ...
    async cancel(id, actorId, reason) {
        const row = await this.prisma.confirmedInteraction.findUnique({ where: { id } });
        if (!row)
            throw new NotFoundException('Взаимодействие не найдено');
        if (row.createdById !== actorId && row.counterpartId !== actorId)
            throw new ForbiddenException('Нет доступа к этому взаимодействию');
        if (row.status === InteractionStatus.COMPLETED)
            throw new BadRequestException('Завершённое взаимодействие нельзя отменить');
        if (row.status === InteractionStatus.CANCELLED)
            return { ok: true };
    ...
    async review(id, actorId, verdict, bodyInput, evidenceMediaId) {
        const row = await this.prisma.confirmedInteraction.findUnique({ where: { id } });
        if (!row)
            throw new NotFoundException('Взаимодействие не найдено');

...
export function promotionMultiplier(activePlaces, demandPercentPerOccupied) {
    return 1 + (Math.max(0, activePlaces) * Math.max(0, demandPercentPerOccupied)) / 100;
}
export function promotionQuoteBreakdown(basePerDay, durationDays, activePlaces, demandPercentPerOccupied) {
...
export function isWithinPromotionRefundGrace(startsAt, now = new Date(), graceMinutes = PROMOTION_REFUND_GRACE_MINUTES) {
    const elapsedMs = Math.max(0, now.getTime() - startsAt.getTime());
    return elapsedMs <= graceMinutes * 60_000;
}
export function blocksDuplicatePromotion(status, endsAt, now = new Date()) {

...
import {
  CommunityContentActionType,
  CommunityRoleType,
  CommunityStructureStatus,
  GlobalRole,
  PublicationFormat,
  PublicationStatus,
  PublicationType,
...
export class CommunityManagementService {
  ...
  private async community(slug: string) {
  ...
  private async ancestorIds(communityId: string) {
  ...
  private async permission(user: User, communityId: string) {
    ...
    if (!role) throw new ForbiddenException('Кабинет доступен только действующей команде сообщества');
    return { level: role.role === CommunityRoleType.CURATOR ? 'CURATOR' as const : role.role === CommunityRoleType.ASSISTANT ? 'ASSISTANT' as const : 'MODERATOR' as const, role };
  }

  private requireLeadership(level: 'ADMIN' | 'CURATOR' | 'ASSISTANT' | 'MODERATOR') {
    if (!['ADMIN', 'CURATOR', 'ASSISTANT'].includes(level)) throw new ForbiddenException('Для этого действия нужны полномочия куратора или помощника');
  }

  private requireCurator(level: 'ADMIN' | 'CURATOR' | 'ASSISTANT' | 'MODERATOR') {
    if (!['ADMIN', 'CURATOR'].includes(level)) throw new ForbiddenException('Для этого действия нужны полномочия куратора');
  }

  async dashboard(user: User, slug: string) {
  ...
  async createReport(user: User, slug: string, dto: CreateCommunityReportDto) {
    const community = await this.community(slug);
    const access = await this.permission(user, community.id);
    this.requireLeadership(access.level);
    const periodStart = new Date(dto.periodStart);
    const periodEnd = new Date(dto.periodEnd);
    if (periodEnd <= periodStart || periodEnd > new Date()) throw new BadRequestException('Период отчёта указан неверно');
    if (periodEnd.getTime() - periodStart.getTime() > 100 * DAY) throw new BadRequestException('Один отчёт не должен охватывать больше 100 дней');
  ...
  async createStructureProposal(user: User, slug: string, dto: CreateStructureProposalDto) {
    const community = await this.community(slug);
    const access = await this.permission(user, community.id);
    this.requireLeadership(access.level);
    const target = dto.targetCommunitySlug ? await this.prisma.community.findUnique({ where: { slug: dto.targetCommunitySlug } }) : null;
    if (dto.targetCommunitySlug && !target) throw new NotFoundException('Целевое сообщество не найдено');
  ...
  async resolveStructureProposal(user: User, slug: string, id: string, dto: ResolveStructureProposalDto) {
    const community = await this.community(slug);
    const access = await this.permission(user, community.id);
    if (access.level !== 'ADMIN') throw new ForbiddenException('Структурные изменения утверждает центральная администрация');
    ...
    if (!proposal) throw new NotFoundException('Открытое предложение не найдено');
  ...
  async invite(user: User, slug: string, dto: InviteCommunityRoleDto) {
    const community = await this.community(slug);
    const access = await this.permission(user, community.id);
    this.requireCurator(access.level);
    if (dto.role === CommunityRoleType.CURATOR && access.level !== 'ADMIN') throw new ForbiddenException('Нового куратора назначает центральная администрация');
    const invitedUser = await this.prisma.user.findUnique({ where: { username: dto.username.trim().toLowerCase() } });
    if (!invitedUser) throw new NotFoundException('Пользователь не найден');
    if (invitedUser.id === user.id) throw new BadRequestException('Нельзя пригласить самого себя');
    ...
    if (existingRole) throw new BadRequestException('У пользователя уже есть эта роль');
    ...
    if (activeInvite) throw new BadRequestException('Такое приглашение уже отправлено');
  ...
  async contentAction(user: User, slug: string, publicationSlug: string, dto: CommunityContentActionDto) {
    const community = await this.community(slug);
    ...
    switch (dto.action) {
      case CommunityContentActionType.PIN: {
        ...
        if (![1, 3, 7, 30].includes(days)) throw new BadRequestException('Закрепление доступно на 1, 3, 7 или 30 дней');
        update = { pinnedUntil: new Date(now.getTime() + days * DAY) }; metadata = { days }; break;
      }
      case CommunityContentActionType.UNPIN: update = { pinnedUntil: null }; break;
      case CommunityContentActionType.CLOSE: update = { closedAt: now, closedReason: dto.note.trim() }; break;
      case CommunityContentActionType.REOPEN: update = { closedAt: null, closedReason: null }; break;
      case CommunityContentActionType.MARK_OFFICIAL: update = { isOfficial: true }; break;
      case CommunityContentActionType.UNMARK_OFFICIAL: update = { isOfficial: false }; break;
      case CommunityContentActionType.MARK_SOLVED: update = { isSolved: true }; break;
      case CommunityContentActionType.UNMARK_SOLVED: update = { isSolved: false }; break;
      case CommunityContentActionType.MOVE: {
        if (!dto.targetCommunitySlug) throw new BadRequestException('Выберите целевое сообщество');

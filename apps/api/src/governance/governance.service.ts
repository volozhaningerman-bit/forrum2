...
import { CommunityRoleType, GlobalRole, PollKind, PollStatus, ProposalStatus, VoteClass, type User } from '../generated/prisma/client.js';
...
export class GovernanceService {
  constructor(private readonly prisma: PrismaService, private readonly communities: CommunitiesService, private readonly notifications: NotificationsService) {}
  ...
  async createProposal(userId: string, dto: CreateProposalDto) {
    ...
    if (dto.parentSlug && !parent) throw new NotFoundException('Родительское сообщество не найдено');
  ...
  async createCuratorApplication(userId: string, dto: CreateCuratorApplicationDto) {
  ...
  async toggleSupport(userId: string, proposalId: string) {
    ...
    if (existing) { await this.prisma.proposalSupport.delete({ where: { proposalId_userId: { proposalId, userId } } }); return { supported: false }; }
  ...
  async createPoll(communitySlug: string, user: User, dto: CreatePollDto) {
    ...
    if (!(await this.canCreatePoll(user, community.id))) throw new ForbiddenException('Опросы создаёт куратор или администрация');
    const closesAt = new Date(dto.closesAt);
    if (closesAt.getTime() < Date.now() + 60 * 60 * 1000 || closesAt.getTime() > Date.now() + 90 * 86400000) throw new BadRequestException('Срок опроса должен быть от 1 часа до 90 дней');
    const options = dto.options.map((option) => option.trim()).filter(Boolean);
    if (new Set(options.map((x) => x.toLowerCase())).size !== options.length) throw new BadRequestException('Варианты не должны повторяться');
  ...
  async castVote(user: User, pollId: string, optionId: string) {
    const poll = await this.prisma.communityPoll.findUnique({ where: { id: pollId }, include: { options: true } });
    if (!poll || poll.status !== PollStatus.OPEN || poll.closesAt <= new Date()) throw new BadRequestException('Опрос закрыт');
    ...
    if (voteClass === VoteClass.ADVISORY && !poll.allowAdvisory) throw new ForbiddenException('Для этого голосования аккаунт пока не соответствует условиям решающего голоса');
  ...
  async closePoll(user: User, pollId: string, resultNote: string) {
    ...
    if (poll.status !== PollStatus.OPEN) throw new BadRequestException('Голосование уже закрыто');
    if (poll.createdById !== user.id && !(await this.canCreatePoll(user, poll.communityId))) throw new ForbiddenException('Закрыть голосование может автор или команда сообщества');

...
import { CommunityRoleType, GlobalRole, type NotifyLevel } from '../generated/prisma/client.js';
...
export class CommunitiesService {
  ...
  async get(slug: string, userId?: string) {
    ...
    if (userId) {
      ...
      if (viewer && [GlobalRole.ADMIN, GlobalRole.OWNER].includes(viewer.role)) canManage = true;
        ...
        let guard = 0;
        while (parentId && guard < 20) { roleScope.push(parentId); parentId = parentById.get(parentId) ?? null; guard += 1; }
  ...
  async updateSubscription(slug: string, userId: string, notifyLevel: NotifyLevel) {
    const community = await this.prisma.community.findUnique({ where: { slug } });
    if (!community) throw new NotFoundException('Сообщество не найдено');
    ...
    if (!subscription) throw new NotFoundException('Сначала подпишитесь на сообщество');
  ...
  async create(dto: CreateCommunityDto, creatorId: string) {
    ...
    if (existing) throw new ConflictException('Такой адрес сообщества уже занят');

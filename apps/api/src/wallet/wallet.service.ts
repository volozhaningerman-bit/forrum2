...
import type { PromotionPricing } from './pricing.js';
...
export class WalletService {
  ...
  private async pricing(client: PrismaService | any = this.prisma) {
  ...
  private parameters(type: PromotionType, pricing: Awaited<ReturnType<WalletService['pricing']>>) {
  ...
  private async expireOrders(client: PrismaService | any = this.prisma, now = new Date()) {
  ...
  private async refreshPinnedUntil(client: any, publicationId: string, now = new Date()) {
  ...
  async quote(communitySlug: string, type: PromotionType, durationDays: number) {
    if (![1, 3, 7, 30].includes(durationDays)) throw new BadRequestException('Доступные сроки: 1, 3, 7 или 30 дней');
  ...
  async purchase(userId: string, communitySlug: string, publicationSlug: string, type: PromotionType, durationDays: number) {
    ...
    if (publication.authorId !== userId) throw new ForbiddenException('Продвигать можно только свою публикацию');
    if (publication.community.slug !== communitySlug) throw new BadRequestException('Публикация находится в другом сообществе');
    const initialQuote = await this.quote(communitySlug, type, durationDays);
    if (!initialQuote.available) throw new BadRequestException('Лимит мест исчерпан');
    return this.prisma.$transaction(async (tx) => {
      ...
      if (duplicate) throw new BadRequestException('Для этой публикации уже действует выбранный формат продвижения');
      ...
      const { limit, basePerDay, demandPercent } = this.parameters(type, pricing);
      if (active >= limit) throw new BadRequestException('Последнее место уже занято');
      ...
      if (wallet.balance < price) throw new BadRequestException('Недостаточно баллов');
  ...
  async cancel(userId: string, orderId: string, reason: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!order) throw new NotFoundException('Продвижение не найдено');
      if (order.userId !== userId) throw new ForbiddenException('Нельзя управлять чужим продвижением');
      if (order.status !== PromotionOrderStatus.ACTIVE) throw new BadRequestException('Это продвижение уже завершено');
      if (order.endsAt <= now) {
        await tx.promotionOrder.update({ where: { id: order.id }, data: { status: PromotionOrderStatus.EXPIRED } });
        if (order.type === PromotionType.PIN) await this.refreshPinnedUntil(tx, order.publicationId, now);
  ...
  async adminRefund(actorId: string, orderId: string, reason: string) {
    return this.prisma.$transaction(async (tx) => {
      await tx.$queryRaw`SELECT pg_advisory_xact_lock(hashtext(${`promotion:${orderId}`}))`;
      const order = await tx.promotionOrder.findUnique({ where: { id: orderId } });
      if (!order) throw new NotFoundException('Продвижение не найдено');
      if (order.status === PromotionOrderStatus.REFUNDED) throw new BadRequestException('Возврат уже выполнен');
      ...
      if (order.type === PromotionType.PIN) await this.refreshPinnedUntil(tx, order.publicationId);
  ...
  async grant(actorId: string, usernameInput: string, amount: number, description: string) {

...
export class TagsService {
  ...
  async get(slugInput: string, userId?: string) {
    ...
    if (!tag) throw new NotFoundException('Хэштег не найден');
  ...
  async subscribe(slugInput: string, userId: string) {
  ...
  async unsubscribe(slugInput: string, userId: string) {
    const tag = await this.prisma.tag.findUnique({ where: { slug: normalizeTag(slugInput) } });
    if (!tag) throw new NotFoundException('Хэштег не найден');
    const existing = await this.prisma.tagSubscription.findUnique({ where: { userId_tagId: { userId, tagId: tag.id } } });
    if (existing) await this.prisma.$transaction([

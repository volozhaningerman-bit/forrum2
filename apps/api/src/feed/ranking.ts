...
export function calculateFeedScore(
  ...
  const recentCommentCount = input.recentCommentCount ?? 0; // FORRUM_FEED_RANKING_COMPAT_V8
  const ageHours = Math.max(
    0,
    (input.nowMs - input.createdAtMs) / 3_600_000,
  );
  const activityHours = Math.max(
    0,
    (input.nowMs - input.lastActivityAtMs) / 3_600_000,
  );
  const engagementRaw =
  ...
  const tagAffinity = Math.min(
    28,
    input.matchingTagCount * 14,
  );
  const popularBonus = input.commentCount >= 5 ? 8 : 0;
  const pinBonus = input.isPinned ? 10 : 0;
  const personallyRelevant = Boolean(
    subscription || authorFollow || tagAffinity,
  );
  const discussed =
  ...
  if (input.mode === 'popular') {
    ...
    const ongoingDiscussion =
      Math.min(input.commentCount, 50) * 7;
    const recentActivity =
      Math.max(0, 72 - activityHours * 3);
    const supportingSignals =
      input.reactionCount * 3 +
      input.bookmarkCount * 2 +

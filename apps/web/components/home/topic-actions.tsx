...
export function TopicActions({item,demo,guest=false}:{item:PublicationCardData;demo:boolean;guest?:boolean}){
 ...
 return <><time className="forum-posted" dateTime={item.createdAt} title={new Date(item.createdAt).toLocaleString('ru-RU',{timeZone:'UTC'})+' UTC'}>{postedAt(item.createdAt,now)}</time><div className="forum-topic-actions" ref={root} onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node))setOpen(false);}}>
  <button ref={trigger} type="button" className={`forum-more-trigger${saved ? ' is-saved' : ''}`} title={saved ? 'Тема в избранном — открыть действия' : 'Действия с темой'} aria-label={`Действия с темой: ${item.title || 'Без заголовка'}`} aria-expanded={open} aria-controls={id} onClick={()=>{setNotice('');setOpen(!open);}}>{saved && <span className="forum-saved-dot" aria-label="В избранном"/>}⋮</button>
  {open && <div className="forum-actions-popover" id={id}><button type="button" onClick={()=>void share()}><span aria-hidden="true">↗</span><span>Поделиться</span></button>{guest && !demo ? <a className="forum-action-login" href="/login">Войти, чтобы сохранить или пожаловаться</a> : <><button type="button" disabled={busy||demo} onClick={()=>void bookmark()}><span aria-hidden="true">☆</span><span>{saved?'Убрать из избранного':'В избранное'}</span></button><button type="button" className="forum-report-action" disabled={demo} onClick={()=>{setOpen(false);setReportError('');dialog.current?.showModal();}}><span aria-hidden="true">⚑</span><span>Пожаловаться</span></button></>}{demo && <small>Действия с аккаунтом доступны на форуме.</small>}</div>}

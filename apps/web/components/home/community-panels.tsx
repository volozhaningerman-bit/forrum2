...
export function CommunityPanels({overview,unavailable,news}:{overview?:HomeOverview;unavailable:boolean;news:PublicationCardData[]}) {
 ...
 return <aside className="forum-right" aria-label="Обзор сообщества">
  {topics.length > 0 && <section className="forum-panel forum-topics-ranking"><header><h2>Сейчас обсуждают</h2></header><p className="forum-ranking-note">Новые ответы за последние 24 часа</p>
  ...
  {authors.length > 0 && <section className="forum-panel"><header><h2>Люди недели <small>за 7 дней</small></h2></header><p className="forum-ranking-note">По числу тем и ответов</p>
  ...
  {!topics.length && !authors.length && <section className="forum-panel forum-start"><span className="forum-eyebrow">Ваш следующий шаг</span><h2>Начните с одной идеи</h2><p>Покажите результат, спросите совет или помогите тому, кто только начинает.</p><Link className="forum-start-primary" href="/create?intent=result">Поделиться работой <span aria-hidden="true">↗</span></Link><Link href="/?tab=unanswered">Найти вопрос без ответа →</Link>{unavailable && <small role="status">Активность временно не обновляется.</small>}</section>}

...
type Data = { days: number; start: string; endExclusive: string; generatedAt: string; series: Point[]; totals: Record<'users' | 'publications' | 'comments', Metric> };
...
export default function AnalyticsView({ compact = false }: { compact?: boolean }) {
  ...
  useEffect(() => { let active = true; setData(null); setError(''); api<Data>(`/admin/analytics?days=${days}`).then(v => { if (active) setData(v); }).catch(e => { if (active) setError(e.message); }); return () => { active = false; }; }, [days, refresh]);

...
type Tab = 'overview' | 'communities' | 'roles' | 'promotion' | 'moderation' | 'system' | 'balance';
...
export default function LegacyPanel({ section }: { section: Tab }) {
  ...
  async function runAction(fn:()=>Promise<void>){if(actionLock.current)return;actionLock.current=true;setSaving(true);try{await fn();}finally{actionLock.current=false;setSaving(false);}}
  const [dirty,setDirty]=useState(false);
  useEffect(()=>{const unload=(e:BeforeUnloadEvent)=>{if(dirty){e.preventDefault();e.returnValue='';}};const leave=(e:MouseEvent)=>{if(dirty&&(e.target as Element).closest('a[href]')&&!window.confirm('Есть несохранённые изменения. Перейти без сохранения?')){e.preventDefault();e.stopPropagation();}};window.addEventListener('beforeunload',unload);document.addEventListener('click',leave,true);return()=>{window.removeEventListener('beforeunload',unload);document.removeEventListener('click',leave,true);};},[dirty]);
  const [saving, setSaving] = useState(false);
  async function submitForm(event: FormEvent<HTMLFormElement>, handler: (event: FormEvent<HTMLFormElement>) => Promise<void>) {
    event.preventDefault();
    if (saving) return;
    setSaving(true);
    try { await handler(event); } finally { setSaving(false); }
  }
  ...
  const load = async () => {
    try {
      ...
      const [dashboardData, reportsData, promotionData, promotionOrdersData, backupData, rolesData, communitiesData] = await Promise.all([
        tab === 'overview' ? api<Dashboard>('/admin/dashboard') : Promise.resolve(null), ['overview', 'moderation'].includes(tab) ? api<Report[]>('/admin/reports') : Promise.resolve([]),
        tab === 'promotion' ? api<PromotionSettings>('/admin/promotion-settings') : Promise.resolve(null), tab === 'promotion' ? api<PromotionOrder[]>('/admin/promotions') : Promise.resolve([]), ['overview', 'system'].includes(tab) ? api<BackupStatus>('/admin/backups/status') : Promise.resolve(null),
        tab === 'roles' ? api<CommunityRole[]>('/admin/community-roles') : Promise.resolve([]), ['roles', 'communities'].includes(tab) ? api<CommunityOption[]>('/communities') : Promise.resolve([]),
      ]);
      ...
      setError(cause instanceof Error ? cause.message : 'Не удалось загрузить панель управления');
    } finally {
      setLoading(false);
    }
  };
  ...
  async function grant(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      showMessage('Тестовые баллы начислены'); formElement.reset();
  ...
  async function createCommunity(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      setError('');
      await api('/communities', { method: 'POST', body: JSON.stringify(Object.fromEntries([...new FormData(formElement)].filter(([, value]) => value !== ''))) });
      showMessage('Категория создана'); formElement.reset(); await load();
  ...
  async function grantCommunityRole(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      setError('');
      await api('/admin/community-roles', { method: 'POST', body: JSON.stringify(Object.fromEntries([...new FormData(formElement)].filter(([, value]) => value !== ''))) });
      showMessage('Роль назначена и записана в историю'); formElement.reset(); await load();
  ...
  return <div className="control-center-page">{dirty&&<p role="status">Есть несохранённые изменения</p>}
    ...
    {!loading && tab === 'communities' && <div className="admin-content-grid">
      <section className="card admin-form-card"><span className="eyebrow">Новая категория</span><h2>Создать категорию</h2><p className="muted">Создавайте только те разделы, для которых уже есть первые авторы и темы.</p><form onChange={()=>setDirty(true)} onSubmit={event => void submitForm(event, createCommunity)}><fieldset disabled={saving}><label>Название<input name="name" minLength={2} maxLength={60} placeholder="Например, Продвижение" required/></label><label>Описание<textarea name="description" minLength={20} maxLength={2000} placeholder="Для кого сообщество и что здесь обсуждают" required/></label><label>Короткое описание<input name="shortDescription" maxLength={180} placeholder="Одна строка для карточки"/></label><label>Адрес страницы <span className="field-hint">необязательно</span><input name="slug" pattern="[a-z0-9-]+" placeholder="promotion"/></label><label>Родительская категория<select name="parentSlug"><option value="">Верхний уровень</option>{flatCommunityOptions.map(item => <option key={item.slug} value={item.slug}>{item.label}</option>)}</select></label><label>Цвет категории<input name="accentColor" type="color" defaultValue="#3157d5"/></label><button className="button">Создать категорию</button></fieldset></form></section>
    ...
    {!loading && tab === 'promotion' && promotion && <div className="admin-promotion-layout">
      <section className="card admin-form-card promotion-settings-card">
        ...
        <form onChange={()=>setDirty(true)} onSubmit={event => void submitForm(event, savePromotion)} className="promotion-form-grid"><fieldset disabled={saving}>
    ...
    {!loading && tab === 'system' && <div className="admin-content-grid"><button className="adm-secondary" onClick={()=>void load()}>Обновить статус</button>
      <section className="card"><span className="eyebrow">Сохранность данных</span><h2>Автоматические резервные копии</h2>{backup ? <><div className={`system-status status-${backup.state}`}><span/>{backup.state === 'ok' ? 'Работает' : backup.state === 'running' ? 'Создаётся' : backup.state === 'error' ? 'Ошибка' : 'Ожидание'}</div><p><strong>{backup.message}</strong></p>{(!backup.updatedAt||!Number.isFinite(Date.parse(backup.updatedAt))||Date.now()-Date.parse(backup.updatedAt)>(backup.intervalHours??12)*7200000)&&<p className="adm-error">Статус отсутствует или устарел. Проверьте службу резервного копирования.</p>}<dl className="system-detail-list"><div><dt>Последняя копия</dt><dd>{backup.lastBackup || 'ещё не создана'}</dd></div><div><dt>Обновление статуса</dt><dd>{backup.updatedAt ? formatRelativeTime(backup.updatedAt) : 'нет данных'}</dd></div><div><dt>Интервал</dt><dd>{backup.intervalHours ?? 12} часов</dd></div><div><dt>Хранится</dt><dd>{backup.retentionCount ?? 28} комплектов</dd></div></dl></> : <div className="empty-state"><strong>Нет данных</strong><span>Сервис резервного копирования ещё не сообщил статус.</span></div>}<p className="muted small-text">Показан статус, полученный от службы резервного копирования. Если данных нет, проверьте её настройку на сервере. Отсутствие статуса не означает, что копия создана.</p></section>

    </div>}
    {!loading && tab === 'balance' && <>      <aside className="card admin-form-card"><span className="eyebrow">Тестовая экономика</span><h2>Начислить баллы</h2><p className="muted">Используется только для проверки поднятия, закреплений и будущей кастомизации.</p><form onChange={()=>setDirty(true)} onSubmit={event => void submitForm(event, grant)}><fieldset disabled={saving}><label>Имя пользователя<UserPicker/></label><label>Количество<input name="amount" type="number" min="1" max="10000000" required/></label><label>Причина<input name="description" minLength={3} maxLength={200} placeholder="Например, тестирование продвижения" required/></label><button className="button">Начислить баллы</button></fieldset></form></aside></>}
  </div>;
}

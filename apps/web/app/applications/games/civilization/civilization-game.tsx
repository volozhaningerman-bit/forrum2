    ...
    <div className="civilization-app">
      <div className="civilization-hub">
        ...
        <main className="civ-scene">
          <img className="civ-cave-background" src="/games/civilization/cave-hub-premium.webp" alt="Пещера первобытного лагеря" />
...
function ItemArt({ item, large = false }: { item: Item; large?: boolean }) {
  ...
  const sprite = generated[item.id];
  if (sprite) {
    const [x, y] = sprite;
    return (
      <span
        className={`civ-item-art civ-generated-item ${large ? 'large' : ''}`}
        aria-hidden="true"
        style={{
          '--civ-item-x': `${x * 33.3333}%`,
          '--civ-item-y': `${y * 50}%`,
        } as CSSProperties}
      />
    );
...
function BossArt({ boss, large = false }: { boss: Boss; large?: boolean }) {
  const position = boss.id === 'ape' ? '0%' : boss.id === 'tiger' ? '50%' : '100%';
  return (
    <span
      className={`civ-boss-illustration civ-generated-boss boss-${boss.id} ${large ? 'large' : ''}`}
      aria-hidden="true"
      style={{ '--civ-boss-x': position } as CSSProperties}
    />
  );
}

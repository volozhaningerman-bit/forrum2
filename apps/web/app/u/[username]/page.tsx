...
type ProfileReview = {
  id: string; verdict: 'POSITIVE' | 'NEUTRAL' | 'NEGATIVE'; body: string; createdAt: string;
  author: { username: string; displayName: string; avatarUrl: string | null };
...
type Profile = {
  ...
  wallPrivacy: 'EVERYONE' | 'FOLLOWERS' | 'ONLY_ME'; wallRestricted: boolean; canWriteWall: boolean; canStartInteraction: boolean;
  ...
  portfolio: ProfilePortfolioItem[]; reviews: ProfileReview[]; reviewSummary: { positive: number; neutral: number; negative: number }; wall: WallPost[];
};

type Tab = 'wall' | 'useful' | 'portfolio' | 'publications' | 'achievements' | 'reviews';
...
export default function ProfilePage() {
  ...
  async function publishWallPost(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (wallText.trim().length < 2) return; setSubmittingWall(true);
    try {
      setError(''); await api(`/users/${username}/wall`, { method: 'POST', body: JSON.stringify({ body: wallText }) });
      setWallText(''); setNotice('Запись опубликована'); await load();
  ...
  return <div className="profile-page-shell">
    <section className="profile-hero">
      ...
      <div className="profile-main-card">
        <div className="profile-main-row">
          ...
          <div className="profile-actions-modern">{profile.isSelf ? <><Link className="button secondary" href="/settings">Настроить профиль</Link><Link className="button ghost" href="/interactions">Взаимодействия</Link></> : <><button type="button" className={`button ${profile.isFollowing ? 'secondary' : ''}`} onClick={follow}>{profile.isFollowing ? 'Вы подписаны' : 'Подписаться'}</button><Link className="button ghost" href={`/messages?to=${encodeURIComponent(profile.username)}`}><MessageIcon/> Написать</Link></>}</div>
        </div>
    ...
    <nav className="profile-tabs" aria-label="Разделы профиля">
      ...
      <button type="button" className={tab === 'useful' ? 'active' : ''} onClick={() => setTab('useful')}>Полезное</button>
      <button type="button" className={tab === 'portfolio' ? 'active' : ''} onClick={() => setTab('portfolio')}>Проекты и услуги</button>
    ...
    <div className="profile-layout-modern">
      <main>
        {tab === 'wall' && <>
          {profile.canWriteWall && <section className="wall-composer-card"><div className="wall-composer-head"><Avatar name={profile.isSelf ? profile.displayName : 'Вы'} size={42} url={profile.isSelf ? profile.avatarUrl : null}/><div><strong>{profile.isSelf ? 'Новая запись на стене' : `Написать на стене ${profile.displayName}`}</strong><span>Короткое обновление, мысль или поздравление</span></div></div><form onSubmit={publishWallPost}><textarea value={wallText} onChange={(event: ChangeEvent<HTMLTextAreaElement>) => setWallText(event.target.value)} minLength={2} maxLength={2000} placeholder="Что хотите рассказать?" required/><div className="composer-footer"><span>{wallText.length}/2000</span><button className="button" disabled={submittingWall || wallText.trim().length < 2}>{submittingWall ? 'Публикуем…' : 'Опубликовать'}</button></div></form></section>}
        ...
        {tab === 'reviews' && <div className="profile-reviews-section">
          ...
          <div className="verified-review-list">{profile.reviews.map((review) => <article className={`verified-review verdict-${review.verdict.toLowerCase()}`} key={review.id}><header><Avatar name={review.author.displayName} size={42} url={review.author.avatarUrl}/><div><Link href={`/u/${review.author.username}`}><strong>{review.author.displayName}</strong></Link><span>@{review.author.username} · {formatRelativeTime(review.createdAt)}</span></div><span className="verified-review-badge"><ShieldIcon/> {verdictNames[review.verdict]}</span></header><p>{review.body}</p><footer><span>{interactionTypeNames[review.interaction.type] ?? review.interaction.type}: {review.interaction.title}</span>{review.interaction.community && <Link href={`/communities/${review.interaction.community.slug}`}>{review.interaction.community.name}</Link>}{review.interaction.publication && <Link href={`/p/${review.interaction.publication.slug}`}>Связанная публикация</Link>}</footer></article>)}{!profile.reviews.length && <div className="empty-state"><strong>Подтверждённых отзывов пока нет</strong><span>Отзывы появятся только после завершённых взаимодействий, подтверждённых обеими сторонами.</span></div>}</div>
        </div>}
      </main>

      <aside className="profile-sidebar-modern">
        <section className="card trust-card"><span className="eyebrow">Контекстное доверие</span><div className="trust-level"><ShieldIcon/><strong>{profile.trustSummary.label}</strong></div><p>{profile.trustSummary.detail}</p>{profile.trustSummary.community && <Link className="text-link" href={`/communities/${profile.trustSummary.community.slug}`}>{profile.trustSummary.community.name}</Link>}<p className="trust-explainer">Это не процент надёжности и не гарантия сделки. Уровень описывает только видимый вклад в конкретном сообществе.</p></section>
        {profile.localTrust.length > 0 && <section className="card"><h3>Доверие по сообществам</h3><div className="local-trust-list">{profile.localTrust.slice(0, 5).map((trust) => <Link href={`/communities/${trust.community.slug}`} key={trust.community.id}><div><strong>{trust.community.name}</strong><span>{trust.label}</span></div><small>{trust.evidence.slice(0, 2).join(' · ') || 'История формируется'}</small></Link>)}</div></section>}
        <section className="card"><h3>Сообщества</h3><div className="profile-community-list">{profile.communities.map((community) => <Link key={community.slug} href={`/communities/${community.slug}`}><span>{community.name.slice(0, 1)}</span><strong>{community.name}</strong></Link>)}{!profile.communities.length && <p className="muted">Подписок на сообщества пока нет.</p>}</div></section>
        ...
        <section className="card gifts-card"><h3>Подарки</h3><div className="compact-empty"><span>Подарков пока нет</span><small>Каталог подарков будет связан с Мастерской после тестирования основной механики.</small></div><Link className="text-link" href="/workshop">Открыть Мастерскую</Link></section>
      </aside>
    </div>
  </div>;
}

...
export default function AdminWorkshop() {
  const queue=useQueue<Item>('workshop'); const {items,error,setError,load}=queue;
  ...
  async function review(id: string, status: 'PUBLISHED' | 'REJECTED') {
    ...
    if (!note || note.length < 3) { setError('Добавьте комментарий (от 3 символов) к проверке'); return; }
    try {
      setError('');
  ...
  return <div className="admin-review-page">
    <div className="section-title"><div><h1>Модерация Мастерской</h1><p className="muted">Оценка пользователей помогает найти сильные работы, но не заменяет проверку качества и авторских прав.</p></div><Link className="button ghost" href="/admin">Обзор форума</Link></div>
    {message && <div className="success-box">{message}</div>}
    <QueueFrame queue={queue}><div className="workshop-review-grid">{items.map((item) => <article className="workshop-review-card" key={item.id}>
